...
ExecButton1.addEventListener("click", () => {
  ...
  document.querySelector("#q-sqrt-result").innerText = wasmExports.q_rsqrt(value);
})
...
function createSteps() {
  ...
  addStep(3, "p", `On va alors pouvoir exécuter notre algorithme : $$\\beta = \\frac{1}{\\sqrt{\\alpha}} = \\frac{1}{\\sqrt{${alpha}}} = q\\_rsqrt(\\alpha) = ${beta}$$`);
  let vecW = new Vec2D(x * beta, y * beta,false);
  addStep(4, "p", `Ainsi, avec ce coefficient \\(\\beta\\), on va pouvoir trouver notre vecteur normalisé \\(\\vec{w}\\) : 
  $$\\vec{w} = ${LaTeXVector("x_\\vec{v}\\times\\beta","y_\\vec{v}\\times\\beta")} = ${LaTeXVector(`${x}\\times\\beta`,`${y}\\times\\beta`)} = ${vecW.toLaTeX()}$$`);
